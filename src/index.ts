...
class MarkCore {
  ...
  constructor(container: any, options: any) {
    ...
    // UI plugins
    if (MarkCore.plugins.length) {
      for (const { plugin: Plugin, options: opts } of MarkCore.plugins) {
        //@ts-ignore
  ...
  setMarkdown(markdown?: any, cursor?: any, isRenderCursor = true) {
  ...
  setFont({ fontSize, lineHeight }: any) {
    if (fontSize) {
  ...
  setListIndentation(listIndentation: any) {
  ...
  replace(value: any, opt: any) {
    this.contentState.replace(value, opt)
    this.contentState.render(false)
    return this.contentState.searchMatches
  }

  find(action: any /* pre or next */) {
  ...
  setOptions(options: any, needRender = false) {
   ...
   */
  _replaceCurrentWordInlineUnsafe(word: any, replacement: any) {
    // __MARKTEXT_PATCH__
 ...
 */
function getContainer(originContainer: any, options: any) {
  ...
  Array.from(attrs).forEach((attr: any) => {
