...
const turnSoftBreakToSpan = (html: any) => {
  const parser = new DOMParser();
  const doc = parser.parseFromString(
    `<x-mt id="turn-root">${html}</x-mt>`,
    "text/html"
  );
  const root: any = doc.querySelector("#turn-root");
  const travel = (childNodes: any) => {
...
const importRegister = (ContentState: any) => {
  ...
  ContentState.prototype.markdownToState = function (markdown: any) {
    // mock a root block...
    const rootState = {
      key: null,
      type: "root",
  ...
  ContentState.prototype.htmlToMarkdown = function (html: any, keeps = []) {
  ...
  // turn html to blocks
  ContentState.prototype.html2State = function (html: any) {
  ...
  ContentState.prototype.addCursorToMarkdown = function (
    markdown: any,
    cursor: any
  ) {
  ...
  ContentState.prototype.importCursor = function (hasCursor: any) {
    // set cursor
    const cursor: any = {
      anchor: null,
      focus: null,
