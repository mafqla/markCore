...
class ImageSelector extends BaseFloat {
  ...
  inputHandler(event: any, type: any) {
  ...
  replaceImageAsync = async ({ alt, src, title }: any) => {
  ...
  renderBody = () => {
    ...
    if (tab === 'select') {
      ...
      if (this.loading) {
        ...
        const photos = this.photoList.map((photo: any) => {
