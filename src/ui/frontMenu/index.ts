...
class FrontMenu extends BaseFloat {
  ...
  listen() {
    ...
    eventCenter.subscribe(
      'muya-front-menu',
      ({ reference, outmostBlock, startBlock, endBlock }: any) => {
        if (reference) {
  ...
  renderSubMenu(subMenu: any) {
    ...
    const children = subMenu.map((menuItem: any) => {
    ...
    if (
      ...
      MAX_SUBMENU_HEIGHT - (ITEM_HEIGHT + PADDING)
    ) {
  ...
  render() {
    const { oldVnode, frontMenuContainer, outmostBlock, startBlock, endBlock } =
      this
  ...
  selectItem(event: any, { label }: any) {
