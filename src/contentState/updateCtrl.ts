...
const updateCtrl = (ContentState: any) => {
   ...
   */
  ContentState.prototype.checkInlineUpdate = function (block: any) {
    ...
    switch (true) {
      case !!hr &&
        new Set(hr.split("").filter((i: any) => /\S/.test(i))).size === 1:
  ...
  // Thematic break
  ContentState.prototype.updateThematicBreak = function (
    block: any,
    marker: any,
    line: any
  ) {
    ...
    const preParagraphLines: any = [];
    let thematicLine = "";
    const postParagraphLines: any = [];
  ...
  ContentState.prototype.updateList = function (
    ...
    if (
      ...
      partChildren.forEach((b: any) => this.appendChild(preSibling, b));
      this.removeBlock(nextSibling);
      this.removeBlock(block);
      const isLooseListItem = preSibling.children.some(
        (c: any) => c.isLooseListItem
      );
      preSibling.children.forEach(
        (c: any) => (c.isLooseListItem = isLooseListItem)
      );
    } else if (
      preSibling &&
      this.checkSameMarkerOrDelimiter(preSibling, bulletMarkerOrDelimiter)
      ...
      if (wrapperTag === "ol") {
        const start: any = cleanMarker ? cleanMarker.slice(0, -1) : 1;
  ...
  ContentState.prototype.updateAtxHeader = function (
    block: any,
    header: any,
    line: any
  ) {
  ...
  ContentState.prototype.updateSetextHeader = function (
    block: any,
    marker: any,
    line: any
  ) {
  ...
  ContentState.prototype.updateBlockQuote = function (block: any, line: any) {
    const text = line.text;
    const lines = text.split("\n");
    const preParagraphLines: any = [];
  ...
  ContentState.prototype.updateIndentCode = function (block: any, line: any) {
    const lang = "";
  ...
  ContentState.prototype.updateToParagraph = function (block: any, line: any) {
