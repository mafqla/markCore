const calculateAspects = (tableId: any, barType: any) => {
  const table: any = document.querySelector(`#${tableId}`)
  if (barType === 'bottom') {
    const firstRow = table.querySelector('tr')
    return Array.from(firstRow.children).map((cell: any) => cell.clientWidth)
  } else {
    return Array.from(table.querySelectorAll('tr')).map(
      (row: any) => row.clientHeight
    )
  }
}

export const getAllTableCells = (tableId: any) => {
  const table: any = document.querySelector(`#${tableId}`)
...
export const getIndex = (barType: any, cell: any) => {
...
const tableDragBarCtrl = (ContentState: any) => {
  ContentState.prototype.handleMouseDown = function (event: any) {
  ...
  ContentState.prototype.handleMouseMove = function (event: any) {
    if (!this.dragInfo) {
      return
    }
  ...
  ContentState.prototype.handleMouseUp = function (event: any) {
    const { eventCenter } = this.muya
  ...
  ContentState.prototype.hideUnnecessaryBar = function () {
    ...
    const needHideBar: any = document.querySelector(
      `.ag-drag-handler.${hideClassName}`
    )
    if (needHideBar) {
  ...
  ContentState.prototype.switchTableData = function () {
    ...
    if (index !== curIndex) {
      ...
      if (barType === 'bottom') {
        for (const row of rows) {
          ...
          if (offset > 0) {
            for (i = index; i < curIndex; i++) {
              row.children[i].children[0].text =
                row.children[i + 1].children[0].text
        ...
        let column: any = null
        const temp = rows[index].children.map((cell: any, i: any) => {
        ...
        if (offset > 0) {
          for (i = index; i < curIndex; i++) {
            rows[i].children.forEach((cell: any, ii: any) => {
              cell.children[0].text = rows[i + 1].children[ii].children[0].text
            })
          }
          rows[curIndex].children.forEach((cell: any, i: any) => {
            if (i === column) {
