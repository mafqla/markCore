...
const tableBlockCtrl = (ContentState: any) => {
  ContentState.prototype.createTableInFigure = function (
    ...
    for (i = 0; i < rows; i++) {
      ...
      i === 0
        ? this.appendChild(tHead, rowBlock)
        : this.appendChild(tBody, rowBlock)
      const rowContents: any = tableContents[i]
      for (j = 0; j < columns; j++) {
  ...
  ContentState.prototype.createFigure = function ({ rows, columns }: any) {
  ...
  ContentState.prototype.createTable = function (tableChecker: any) {
  ...
  ContentState.prototype.tableToolBarClick = function (type: any) {
    const {
      start: { key }
    } = this.cursor
    ...
    switch (type) {
      ...
      case 'right': {
        ...
        table.children.forEach((rowContainer: any) => {
          rowContainer.children.forEach((row: any) => {
      ...
      case 'table': {
        ...
        const tableEle: any = document.querySelector(
          `#${figureKey} [data-label=table]`
        )
        const { row = 1, column = 1 } = table // zero base

        const handler = (row: any, column: any) => {
  ...
  ContentState.prototype.editTable = function (
    ...
    if (block.functionType !== 'cellContent') {
      throw new Error(
        'Cursor is not in table block, so you can not insert/edit row/column'
      )
    }
    ...
    if (target === 'row') {
      if (action === 'insert') {
        ...
        if (location === 'previous') {
          ...
          if (cellBlock.type === 'th') {
            ...
            currentRow.children.forEach((cell: any) => (cell.type = 'td'))
        ...
        if (location === 'previous') {
          ...
          if (!currentRow.preSibling) {
            ...
            this.removeBlock(currentRow)
            currentRow.children.forEach((cell: any) => (cell.type = 'th'))
          ...
          if (cellBlock.type === 'th' && tbody.children.length >= 2) {
            ...
            firstRow.children.forEach((cell: any) => (cell.type = 'th'))
            cursorBlock = firstRow.children[columnIndex].children[0]
          }
          if (
            cellBlock.type === 'td' &&
            (currentRow.preSibling || currentRow.nextSibling)
          ) {
            cursorBlock = (
      ...
      if (action === 'insert') {
        ;[...thead.children, ...tbody.children].forEach(tableRow => {
          ...
          tableRow.children.forEach((cell: any, i: any) => {
            cell.column = i
          })
        })
        cursorBlock =
  ...
  ContentState.prototype.getTableBlock = function () {
    ...
    const affiliation = startParents.filter((p: any) => endParents.includes(p))

    if (affiliation.length) {
      const figure = affiliation.find((p: any) => p.type === 'figure')
      return figure
    }
  }

  ContentState.prototype.tableBlockUpdate = function (block: any) {
    const { type } = block
