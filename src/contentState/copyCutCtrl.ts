...
const copyCutCtrl = (ContentState: any) => {
  ContentState.prototype.docCutHandler = function (event: any) {
  ...
  ContentState.prototype.getClipBoardData = function () {
    ...
    const removedElements: any = wrapper.querySelectorAll(
    ...
    const taskListItems: any = wrapper.querySelectorAll('li.ag-task-list-item')
    for (const item of taskListItems) {
      const firstChild = item.firstElementChild
      if (firstChild && firstChild.nodeName !== 'INPUT') {
        const originItem: any = document.querySelector(`#${item.id}`)
    ...
    const images: any = wrapper.querySelectorAll('span.ag-inline-image img')
    ...
    const hrs: any = wrapper.querySelectorAll('[data-role=hr]')
    for (const hr of hrs) {
      hr.replaceWith(document.createElement('hr'))
    }

    const headers: any = wrapper.querySelectorAll('[data-head]')
    ...
    const inlineRuleElements: any = wrapper.querySelectorAll(
    ...
    const aLinks: any = wrapper.querySelectorAll(`.${CLASS_OR_ID.AG_A_LINK}`)
    ...
    const codefense: any = wrapper.querySelectorAll("pre[data-role$='code']")
    ...
    const tightListItem: any = wrapper.querySelectorAll('.ag-tight-list-item')
    ...
    const htmlBlock: any = wrapper.querySelectorAll("figure[data-role='HTML']")
    ...
    const lineBreaks: any = wrapper.querySelectorAll(
    ...
    const mathBlock: any = wrapper.querySelectorAll('figure.ag-container-block')
  ...
  ContentState.prototype.docCopyHandler = function (event: any) {
    const { selectedTableCells } = this
    if (selectedTableCells) {
      event.preventDefault()
  ...
  ContentState.prototype.copyHandler = function (
    event: any,
    type: any,
    copyInfo = null
  ) {
    ...
    switch (type) {
      ...
      case 'copyBlock': {
        const block =
          typeof copyInfo === 'string' ? this.getBlock(copyInfo) : copyInfo
        if (!block) return
      ...
      case 'copyCodeContent': {
        const codeContent: any = copyInfo
