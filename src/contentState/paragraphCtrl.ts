...
//  eg: h1 => 1
//      h2 => 2
const getCurrentLevel = (type: any) => {
  if (/\d/.test(type)) {
    //@ts-ignore
...
const paragraphCtrl = (ContentState: any) => {
  ContentState.prototype.selectionChange = function (cursor: any) {
    ...
    const affiliation = startParents
      .filter((p: any) => endParents.includes(p))
      .filter((p: any) => PARAGRAPH_TYPES.includes(p.type))
  ...
  ContentState.prototype.getCommonParent = function () {
    ...
    const startBlock = this.getBlock(start.key)
    const endBlock = this.getBlock(end.key)
    const startParentKeys = this.getParents(startBlock).map((b: any) => b.key)
    const endParentKeys = this.getParents(endBlock).map((b: any) => b.key)
  ...
  ContentState.prototype.handleListMenu = function (
    paraType: any,
    insertMode: any
  ) {
    const { start, end, affiliation } = this.selectionChange(this.cursor)
    const { orderListDelimiter, bulletListMarker, preferLooseListItem } =
      this.muya.options
    const [blockType, listType] = paraType.split('-')
    const isListed = affiliation
      .slice(0, 3)
      .filter((b: any) => /ul|ol/.test(b.type))
    ...
    if (isListed.length && !insertMode) {
      ...
      if (listType === listBlock.listType) {
        ...
        listItems.forEach((listItem: any) => {
          listItem.children.forEach((itemParagraph: any) => {
      ...
      listBlock.children.forEach((b: any) => (b.listItemType = listType))

      if (listType === 'order') {
        listBlock.start = listBlock.start || 1
        listBlock.children.forEach(
          (b: any) => (b.bulletMarkerOrDelimiter = orderListDelimiter)
        )
      }
      if (
        ...
        listBlock.children.forEach(
          (b: any) => (b.bulletMarkerOrDelimiter = bulletListMarker)
        )
      }
      ...
      if (
        ...
        children.slice(startIndex, endIndex + 1).forEach((child: any) => {
          if (child !== referBlock) {
  ...
  ContentState.prototype.handleQuoteMenu = function (insertMode: any) {
    const { start, end, affiliation } = this.selectionChange(this.cursor)
    let startBlock = this.getBlock(start.key)
    const isBlockQuote = affiliation
      .slice(0, 2)
      .filter((b: any) => /blockquote/.test(b.type))
  ...
  ContentState.prototype.insertContainerBlock = function (
    functionType: any,
    block: any
  ) {
  ...
  ContentState.prototype.showTablePicker = function () {
    ...
    const handler = (rows: any, columns: any) => {
  ...
  ContentState.prototype.insertHtmlBlock = function (block: any) {
    if (block.type === 'span') {
  ...
  ContentState.prototype.updateParagraph = function (
    ...
    switch (paraType) {
      ...
      case 'paragraph': {
        ...
        // \u00A0 is &nbsp;
        const [, hash, partText]: any = /(^ {0,3}#*[ \u00A0]*)([\s\S]*)/.exec(
          text
        )
  ...
  ContentState.prototype.deleteParagraph = function (blockKey: any) {
  ...
  ContentState.prototype.getTypeFromBlock = function (block: any) {
    const { type } = block
