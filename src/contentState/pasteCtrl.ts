...
const pasteCtrl = (ContentState: any) => {
  // check paste type: `MERGE` or `NEWLINE`
  ContentState.prototype.checkPasteType = function (start: any, fragment: any) {
    ...
    if (fragmentType === 'p') {
      ...
      const list =
        listItem && listItem.type === 'li' ? this.getParent(listItem) : null
      if (list) {
        if (
  ...
  ContentState.prototype.checkCopyType = function (html: any, rawText: any) {
    let type = 'normal'
    if (!html && rawText) {
      type = 'copyAsMarkdown'
      const match =
      ...
      if (match && match[1]) {
        ...
        if (tag === 'table' && match.length === 3 && match[2] === 'table') {
          ...
          const tmp: any = document.createElement('table')
  ...
  ContentState.prototype.standardizeHTML = async function (rawHtml: any) {
    ...
    const tempWrapper: any = document.createElement('div')
    tempWrapper.innerHTML = sanitizedHtml

    // Special process for turndown.js, needed for Number app on macOS.
    const tables: any = Array.from(tempWrapper.querySelectorAll('table'))
    for (const table of tables) {
      ...
      const paragraphs: any = Array.from(table.querySelectorAll('p'))
    ...
    const links: any = Array.from(tempWrapper.querySelectorAll('a'))
    for (const link of links) {
      const href = link.getAttribute('href')
      const text = link.textContent
      if (URL_REG.test(href) && href === text) {
        const title: any = await getPageTitle(href)
        if (title) {
  ...
  ContentState.prototype.pasteImage = async function (event: any) {
  ...
  ContentState.prototype.docPasteHandler = async function (event: any) {
  ...
  ContentState.prototype.pasteHandler = async function (
    ...
    const getLastBlock: any = (blocks: any) => {
    ...
    switch (pasteType) {
      case 'MERGE': {
        if (LIST_REG.test(firstFragment.type)) {
          ...
          if (targetListType !== originListType) {
            if (!targetListType) {
              firstFragment.children.forEach(
                (item: any) => (item.isLooseListItem = true)
              )
            } else {
              originList.children.forEach(
                (item: any) => (item.isLooseListItem = true)
              )
            }
          }
          ...
          if (liChildren[0].type === 'p') {
            ...
            if (firstFragmentTail.length) {
              firstFragmentTail.forEach((t: any) => {
                this.appendChild(originList, t)
              })
            }
          } else {
            listItems.forEach((c: any) => {
              this.appendChild(originList, c)
            })
          }
          let target = originList
          tailFragments.forEach((block: any) => {
      ...
      case 'NEWLINE': {
        let target = parent
        stateFragments.forEach((block: any) => {
          this.insertAfter(block, target)
          target = block
        })
