...
const backspaceCtrl = (ContentState: any) => {
  ...
  ContentState.prototype.docBackspaceHandler = function (event: any) {
  ...
  ContentState.prototype.backspaceHandler = function (event: any) {
    ...
    const tableHasContent = (table: any) => {
      const tHead = table.children[0]
      const tBody = table.children[1]
      const tHeadHasContent = tHead.children[0].children.some((th: any) =>
        th.children[0].text.trim()
      )
      const tBodyHasContent = tBody.children.some((row: any) =>
        row.children.some((td: any) => td.children[0].text.trim())
      )
