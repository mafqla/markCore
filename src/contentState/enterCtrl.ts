...
const checkAutoIndent = (text: any, offset: any) => {
  const pairStr = text.substring(offset - 1, offset + 1);
  return /^(\{\}|\[\]|\(\)|><)$/.test(pairStr);
};
const getIndentSpace = (text: any) => {
  const match = /^(\s*)\S/.exec(text);
  return match ? match[1] : "";
};

const enterCtrl = (ContentState: any) => {
  // TODO@jocs this function need opti.
  ContentState.prototype.chopBlockByCursor = function (
    block: any,
    key: any,
    offset: any
  ) {
    ...
    const index = children.findIndex((child: any) => child.key === key);
    const activeLine = this.getBlock(key);
    const { text } = activeLine;
    newBlock.children = children.splice(index + 1);
    newBlock.children.forEach((c: any) => (c.parent = newBlock.key));
  ...
  ContentState.prototype.chopBlock = function (block: any) {
  ...
  ContentState.prototype.createRow = function (row: any, isHeader = false) {
  ...
  ContentState.prototype.createBlockLi = function (paragraphInListItem: any) {
  ...
  ContentState.prototype.enterInEmptyParagraph = function (block: any) {
    ...
    if (parent && /ul|ol|blockquote/.test(parent.type)) {
      ...
      blocksInListItem.forEach((b: any) => this.appendChild(newBlock, b));
      this.removeBlock(block);
  ...
  ContentState.prototype.enterHandler = function (event: any) {
    ...
    const getFirstBlockInNextRow = (row: any) => {
    ...
    switch (true) {
      case left !== 0 && right !== 0: {
        ...
        let { pre, post } = selection.chopHtmlByCursor(paragraph) as any;
    ...
    const getParagraphBlock = (block:any) => {
      if (block.type === "li") {
