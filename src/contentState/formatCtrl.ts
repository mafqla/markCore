...
const getOffset: any = (
  offset: any,
  { range: { start, end }, type, tag, anchor, alt }: any
) => {
...
const clearFormat = (token: any, { start, end }: any) => {
  if (start) {
...
const addFormat = (type: any, block: any, { start, end }: any) => {
  if (
    block.type !== "span" ||
    (block.type === "span" &&
...
const checkTokenIsInlineFormat = (token: any) => {
...
const formatCtrl = (ContentState: any) => {
  ...
  ContentState.prototype.format = function (type: any) {
