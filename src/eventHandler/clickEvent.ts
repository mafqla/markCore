...
class ClickEvent {
  ...
  clickBinding() {
    ...
    const handler = (event: any) => {
      ...
      const imageDelete =
        target.closest('.ag-image-icon-delete') ||
        target.closest('.ag-image-icon-close')
      const mathText = mathRender && mathRender.previousElementSibling
      const rubyText = rubyRender && rubyRender.previousElementSibling
      if (
        markedImageText &&
        markedImageText.classList.contains(CLASS_OR_ID.AG_IMAGE_MARKED_TEXT)
      ) {
      ...
      if (footnoteBackLink) {
        ...
        const identifier = figure.querySelector(
          'span.ag-footnote-input'
        ).textContent
        if (identifier) {
          const footnoteIdentifier = document.querySelector(
            `#noteref-${identifier}`
          )
      ...
      if (target.tagName === 'IMG' && imageWrapper) {
        ...
        const rect = imageWrapper
          .querySelector('.ag-image-container')
          .getBoundingClientRect()
        const reference = {
          getBoundingClientRect() {
            return rect
        ...
        const imageContainer = document.querySelector(
          `${imageSelector} .ag-image-container`
        )
      ...
      if (
        target.closest('div.ag-container-preview') ||
...
function getToolItem(target: any) {
  return target.closest('[data-label]')
}

function selectionText(node: any) {
